...
export function PlanCard({
  ...
  return (
    <div
      className={cn(
        "card-lift relative flex h-full flex-col rounded-3xl border p-7",
        highlight
          ? "border-accent/40 bg-gradient-olive text-primary-foreground shadow-lift"
          : "border-primary/10 bg-card shadow-soft",
      )}
    >
      {highlight ? (
      ...
      <div className="mt-6 flex items-end gap-2">
        ...
        <span className={cn("pb-1.5 text-sm", highlight ? "text-primary-foreground/70" : "text-muted-foreground")}>
          /day
        </span>
      </div>
      ...
      <div
        className={cn(
          ...
          highlight ? "bg-primary-foreground/10 text-accent" : "bg-muted text-primary",
        )}
      >
        {plan.discount_percentage}% discount on eligible services
        <span className={cn("mt-1 block text-xs font-normal", highlight ? "text-primary-foreground/70" : "text-muted-foreground")}>
      ...
      {limited ? (
        <div className="mt-5">
          <div className="flex items-center justify-between text-xs font-semibold">
            ...
            <span className={highlight ? "text-accent" : "text-secondary"}>{occupied} occupied</span>
          </div>
          <Progress value={((occupied / (plan.slot_limit || 1)) * 100) | 0} className="mt-2 h-2" />
        </div>
      ) : null}
      ...
      <ul className="mt-6 flex-1 space-y-2.5 text-sm">
        {plan.benefits.map((b) => (
          <li key={b} className="flex gap-2">
            <Check className={cn("mt-0.5 size-4 shrink-0", highlight ? "text-accent" : "text-secondary")} />
            <span className={highlight ? "text-primary-foreground/85" : "text-foreground/85"}>{b}</span>
          </li>
        ))}
      </ul>
      ...
      <div className="mt-7">
        {full ? (
          <div className="space-y-2">
            ...
            <Button asChild variant={highlight ? "lime" : "hero"} className="w-full">
              <a href={`tel:${supportPhone}`}>Contact Us for Availability</a>
            </Button>
          </div>
        ) : onSubscribe ? (
          <Button
            variant={highlight ? "lime" : "hero"}
            className="w-full"
            disabled={busy}
        ...
        <p className={cn("mt-3 text-center text-[11px]", highlight ? "text-primary-foreground/60" : "text-muted-foreground")}>
